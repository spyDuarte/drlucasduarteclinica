...
export default function Login() {
  ...
  const { login, isLoginLocked, failedLoginAttempts, getLoginLockRemainingTime } = useAuth();
  const navigate = useNavigate();

  useEffect(() => {
    const updateRemaining = () => setLockRemainingMs(getLoginLockRemainingTime());
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isLoginLocked) {
      ...
      setError(`Acesso temporariamente bloqueado. Tente novamente em ${seconds}s.`);
      return;
    }
  ...
  const isSubmitDisabled = isLoading || isLoginLocked;
        ...
        <div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100">
          ...
          {isLoginLocked && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-sm">
              Muitas tentativas inválidas. Novo acesso em <span className="font-semibold">{Math.ceil(lockRemainingMs / 1000)}s</span>.
            </div>
          )}

          {!isLoginLocked && failedLoginAttempts > 0 && (
            <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-lg text-orange-800 text-sm">
              Tentativas inválidas recentes: <span className="font-semibold">{failedLoginAttempts}</span>.
              {' '}Após 5 tentativas, o acesso é bloqueado temporariamente.
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all text-slate-900 placeholder:text-slate-400 disabled:bg-slate-100"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-12 py-2.5 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all text-slate-900 placeholder:text-slate-400 disabled:bg-slate-100"
                  placeholder="••••••••"
                  required
                  disabled={isLoginLocked}
                />
            ...
            <button
              ...
              className="w-full btn-primary py-2.5 text-sm shadow-md shadow-primary-500/10 hover:shadow-lg hover:shadow-primary-500/20 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
